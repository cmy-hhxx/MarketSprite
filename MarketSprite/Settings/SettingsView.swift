...
struct SettingsView: View {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                Group {
                    ...
                    case .data:
                        ...
                        .onAppear {
                            Task { await store.refreshQuoteBarCount() }
                        }
                    }
                }
            }
    ...
    private var watchlistView: some View {
        VStack(alignment: .leading, spacing: 16) {
            pageTitle("观察列表", subtitle: "搜索名称或代码，也可通过 JSON 批量导入")
                        ...
                        HStack(spacing: 10) {
                            ...
                            if let quote = store.monitoredInstrument(for: instrument.id)?.quote {
                                Text(String(format: "%@%.2f%%", quote.changePercent >= 0 ? "+" : "", quote.changePercent))
                                    ...
                                    .foregroundStyle(instrument.market.colorRole(isRising: quote.changePercent >= 0).color)
                            }
        ...
        VStack(alignment: .leading, spacing: 18) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Text("• 已观察标的的分钟线会写入本地 SQLite；A 股收盘后至次日开盘前会标注 B/S 极值。")
                }
                .font(.caption)
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
